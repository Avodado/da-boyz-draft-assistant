...
test("draft-day overlay applies the material ADP moves and remains idempotent", () => {
...
test("availability overlay fixes Jeanty and Judkins without changing breakout inputs", () => {
  const { api } = loadApi();
  ...
  assert.match(jeanty.injury_source, /nfl\.com/);

...
test("install precaches the complete v0.12.7-github-1 release", async () => {
  const harness = makeWorkerHarness();
  let installed;
  harness.listeners.install({ waitUntil(promise) { installed = promise; } });
  await installed;
  assert.equal(vm.runInContext("CACHE", harness.context), "daboyz-draft-assistant-v0.12.7-github-1");
...
test("v0.12.6 cache upgrades to v0.12.7 and remains offline-usable", async () => {
  const harness = makeWorkerHarness();
  const current = vm.runInContext("CACHE", harness.context);
  const old = "daboyz-draft-assistant-v0.12.6-github-1";
  harness.stores.set(old, new Map([["https://example.test/draft/index.html", new FakeResponse(200, "old-index")]]));
  let installed;
  harness.listeners.install({ waitUntil(promise) { installed = promise; } });
  await installed;
  assert.equal(harness.stores.has(current), true);
  await (await harness.caches.open(current)).put("https://example.test/draft/index.html", new FakeResponse(200, "v0.12.7-index"));
  let activated;
  harness.listeners.activate({ waitUntil(promise) { activated = promise; } });
  await activated;
  ...
  assert.equal(response.body, "v0.12.7-index");
});
...
test("hosted update comparison accepts only a newer semantic build", () => {
  ...
  assert.equal(isNewerBuild("v0.12.7", "v0.12.6"), true);
  assert.equal(isNewerBuild("v0.12.6", "v0.12.6"), false);
  assert.equal(isNewerBuild("v0.12.6", "v0.12.5"), true);
  assert.equal(isNewerBuild("v0.12.5", "v0.12.5"), false);
...
function hostedUpdateHarness(meta, currentBuild = "v0.12.6") {
  ...
  const source = appHtml.slice(start, end).replace('const CURRENT_BUILD="v0.12.7"', `const CURRENT_BUILD="${currentBuild}"`);
  vm.runInContext(source, context);
  return { checkHostedUpdate: vm.runInContext("checkHostedUpdate", context), boxes };
}


test("hosted v0.12.7 metadata visibly announces an update from v0.12.6", async () => {
  const harness = hostedUpdateHarness({ build: "v0.12.7" });
  await harness.checkHostedUpdate();
  assert.equal(harness.boxes.updateNotice.style.display, "block");
  assert.equal(harness.boxes.updateNoticeTitle.textContent, "Update available • v0.12.7");
  assert.match(harness.boxes.updateNoticeText.textContent, /Running v0\.12\.6/);
});


test("older or malformed hosted metadata stays hidden", async () => {
  for (const meta of [{ build: "v0.12.6" }, { build: "latest" }, {}, { build: "0.12.7" }]) {
...
test("v0.12.6 draft state is saved intact before the v0.12.7 reload", async () => {
  const draftState = {

...
const CACHE='daboyz-draft-assistant-v0.13.0-github-2';
const INDEX=new URL('./index.html',self.registration.scope).href;
const ASSETS=['./','./index.html','./app.html.gz','./interpretation-layer.js','./version.json','./manifest.webmanifest','./icon-192.png','./icon-512.png','./icon-192.svg','./icon-512.svg'];
...
self.addEventListener('fetch',event=>{
 ...
 if(url.pathname.endsWith('/app.html.gz')||url.pathname.endsWith('/interpretation-layer.js')||url.pathname.endsWith('/version.json')||url.pathname.endsWith('/index.html')){event.respondWith(networkFirst(req));return}

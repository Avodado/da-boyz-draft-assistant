...
function genericPlayerContext(p){let intrinsic=null;try{if(typeof intrinsicScore==='function')intrinsic=num(intrinsicScore(p))}catch(e){}return{...marketPlayerContext(p),intrinsic}}
...
function availableAffinityPlayers(cur,affinity){if(typeof state==='undefined'||!Array.isArray(state.players))return[];return state.players.filter(p=>p&&!p.drafted&&affinity.teams.includes(p.nfl_team)&&isOwnerLegal(cur,p)).map(genericPlayerContext).sort((a,b)=>(a.planningAdp??9999)-(b.planningAdp??9999)||(a.marketRank??9999)-(b.marketRank??9999)||a.name.localeCompare(b.name)).slice(0,5)}
